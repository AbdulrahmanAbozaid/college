...
from utils import contains_negative
from utils import operate_on_tuples
...
class State:
    ...
    def value_of(cls, state):
        if not cls.__is_valid_tuple(state):
            raise ValueError(f"{str(state)} must be a tuple with 3 elements.")
    ...
    def __has_more_cannibals_than_missionaries(self):
        """check if the cannibals have eaten the missionaries or not."""
