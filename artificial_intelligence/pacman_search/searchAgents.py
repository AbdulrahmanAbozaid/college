...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState, costFn=lambda x: 1):
        ...
        bottom, right = self.walls.height - 2, self.walls.width - 2
        self.corners = ((1, 1), (1, bottom), (right, 1), (right, bottom))
    ...
    def getSuccessors(self, state):
        ...
        for action in [
            ...
            if not self.walls[nextx][nexty]:
                ...
                restCorners = tuple(crn for crn in corners if crn != (nextx, nexty))
                nextState = (successor, restCorners)
                cost = 1
                successors.append((nextState, action, cost))
...
def crnDistHeuristic(currentPosition, corners):
    """cumulative manhatten distance
    Args:
        currentPosition: (x, y)
        goals: Corners ((1,1), (1, length), ...)

    Returns: the cumulative manhatten distance
    Description:
        first the cost to the nearest corner
        then the cost from this corner to reach the goal state
        by exploring the rest of the unvisited corners
    """
    x, y = currentPosition
    score = 0
    goals = corners[:]
    for _ in corners:
        # means from (x, y) to g(a, b)
        cordToCost = {((x, y), g): (abs(x - g[0]) + abs(y - g[1])) for g in goals}
        minDist = min(cordToCost.values())
        score += minDist  # cumulative Manhattan distance to nearest corner
        for key, value in cordToCost.items():
            if value == minDist:
                minKey = key
        x, y = minKey[1]
        goals = tuple([g for g in goals if g != (x, y)])
    return score
...
def cornersHeuristic(state, problem):
    ...
    """
    currentPosition, corners = state
    return crnDistHeuristic(currentPosition, corners)
